...
/*
  This root is for my project that depends on puppeteer.
  Since pkg was affecting this method I had to manually update it 
*/
function webpackRoot(){
  var args = Array.prototype.slice.call(arguments,0);
  return path.join.apply(null, [ process.cwd(), './node_modules/puppeteer'].concat(args));  
}
...
class BrowserFetcher {
  ...
  revisionInfo(revision) {
    //const folderPath = this._getFolderPath(revision);
    const folderPath = root('.local-chromium', this._platform + '-' + revision);
    let executablePath = '';
    if (this._platform === 'mac')
      executablePath = root('.local-chromium', this._platform + '-' + revision, 'chrome-mac', 'Chromium.app', 'Contents', 'MacOS', 'Chromium');
    else if (this._platform === 'linux')
      executablePath = root('.local-chromium', this._platform + '-' + revision, 'chrome-linux', 'chrome');
    else if (this._platform === 'win32' || this._platform === 'win64')
      executablePath = root('.local-chromium', this._platform + '-' + revision, 'chrome-win32', 'chrome.exe');
    else
